...
function setupPdfDownload() {
  const btn = document.querySelector("#download-instruction-pdf");
  if (!btn) return;

  btn.addEventListener("click", async () => {
    const pdfUrl = new URL("instrukcja_terenowa_sieweczka.pdf", window.location.href).toString();
    try {
      const response = await fetch(pdfUrl, { method: "HEAD", cache: "no-cache" });
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}`);
      }
      const link = document.createElement("a");
      link.href = pdfUrl;
      link.download = "instrukcja_terenowa_sieweczka.pdf";
      link.rel = "noopener";
      ...
      alert(
        "Nie znaleziono pliku instrukcja_terenowa_sieweczka.pdf. " +
        "Dodaj go do głównego katalogu repozytorium (obok index.html), " +
        "wypchnij na GitHub i odśwież aplikację."
      );
...
const fieldHelpMap = {
  ...
  "#random-pct-sand": "Punkt losowy 1 m² — piasek: definicja i sposób kodowania identyczne jak w gnieździe.",
  "#random-pct-fine-gravel": "Punkt losowy 1 m² — drobny żwir: definicja i sposób kodowania identyczne jak w gnieździe.",
  "#random-pct-coarse": "Punkt losowy 1 m² — gruby żwir/kamienie: definicja i sposób kodowania identyczne jak w gnieździe.",
  "#random-pct-shells": "Punkt losowy 1 m² — muszle: definicja i sposób kodowania identyczne jak w gnieździe.",
  "#random-pct-live-veg": "Punkt losowy 1 m² — roślinność żywa: definicja i sposób kodowania identyczne jak w gnieździe.",
  "#random-pct-dry-veg": "Punkt losowy 1 m² — roślinność sucha: definicja i sposób kodowania identyczne jak w gnieździe.",
  "#random-pct-organic": "Punkt losowy 1 m² — drewno/szczątki organiczne: definicja i sposób kodowania identyczne jak w gnieździe.",
  "#random-pct-anthro": "Punkt losowy 1 m² — obiekty antropogeniczne: definicja i sposób kodowania identyczne jak w gnieździe.",
  ...
  "#pct-sand": "Bufor 15 m — piasek: łączny udział otwartych powierzchni piaszczystych w promieniu 15 m.",
